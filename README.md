...
Manage multiple named instances of the same container type with the `./dev` CLI.

macOS 上の Docker で Claude Code を動かすための開発環境。
名前付きインスタンスで同タイプのコンテナを複数運用でき、`./dev` CLI で管理する。
...
./dev create web-dev my-app

# Add another web development instance (multiple of same type OK)
./dev create web-dev my-api

# Create a sandbox instance
./dev create sandbox experiment
```

#### 3. Connect to a Container

```bash
./dev connect my-app             # Connect to bash
./dev connect my-app claude      # Launch Claude Code directly
```
...
Check auth status with `./dev auth-status`.

### ./dev Command Reference

| Command | Description |
|---------|-------------|
| `./dev create <type> <name>` | Create & start an instance |
| `./dev connect <name> [cmd]` | Connect to container (default: bash) |
| `./dev list` | List all instances (name, type, status) |
| `./dev rm <name> [--volumes]` | Remove instance (`--volumes` to also remove volumes) |
| `./dev purge-shared [vol ...]` | Remove shared volumes (auth, DB data) |
| `./dev start <name>` | Start an instance |
| `./dev stop <name>` | Stop an instance |
| `./dev up` | Start all instances |
| `./dev down` | Stop all containers |
| `./dev build [type]` | Build images (omit type for all) |
| `./dev regenerate` | Regenerate compose.generated.yml from templates |
| `./dev ps` | Show running container status |
| `./dev auth-status [name]` | Check Claude CLI auth status (token expiry) |
| `./dev types` | List available container types |
| `./dev help` | Show help |
...
```bash
./dev connect my-app             # bash に接続
./dev connect my-app claude      # Claude Code を直接起動
```
...
| `./dev list` | 全インスタンス一覧（名前・タイプ・ステータス） |
| `./dev rm <name> [--volumes]` | インスタンス削除（`--volumes` でボリュームも削除） |
| `./dev purge-shared [vol ...]` | 共有ボリューム削除（認証情報・DB データ） |
| `./dev start <name>` | インスタンス起動 |
| `./dev stop <name>` | インスタンス停止 |
| `./dev up` | 全インスタンス一括起動 |
| `./dev down` | 全コンテナ一括停止 |
| `./dev build [type]` | イメージビルド（タイプ省略で全タイプ） |
| `./dev regenerate` | compose.generated.yml をテンプレートから再生成 |
| `./dev ps` | 実行中コンテナの状態 |
| `./dev auth-status [name]` | Claude CLI 認証状態確認（トークン有効期限表示） |
...
> **注意**: `~/project/.mcp.json.chrome-devtool` が既に存在する場合、entrypoint.sh はスキップする。
...
3. 新しい `./dev` で同等のインスタンスを作成:
   ```bash
   ./dev create web-dev web-dev
   ./dev create apple-dev apple-dev
   ./dev create sandbox sandbox
   ```
